...
class AchievementService {
    ...
    private getAchievementLevel(condition: { type: string; value?: number }): string {
    ...
    private calculateProgress(achievement: Achievement, userStats?: any): { progressText: string; percentage: number } {
        const { type, value } = achievement.condition;

        if (!userStats) {
            return {
                progressText: value ? `${value}/${value}` : '1/1',
                percentage: 100
            };
        }
        ...
        switch (type) {
            ...
            case 'complete_lessons':
                ...
                const lessonPercentage = Math.min((completedLessons / targetLessons) * 100, 100);
                return {
            ...
            case 'complete_courses':
                ...
                const coursePercentage = Math.min((completedCourses / targetCourses) * 100, 100);
                return {
            ...
            case 'no_mistake_lesson':
                ...
                const perfectPercentage = Math.min((perfectLessons / targetPerfect) * 100, 100);
                return {
    ...
    async getUserAchievements(userId: string): Promise<UserAchievementResponse> {
        try {
            ...
            const data = await this.makeRequest<UserAchievementResponse>(`/api/user-archivement/${userId}`);
            return data;
        } catch (error) {
    ...
    async getProcessedAchievements(userId: string, userStats?: any): Promise<ProcessedAchievement[]> {
        try {
            ...
            const processedAchievements: ProcessedAchievement[] = response.data.map((userAchievement) => {
                const achievement = userAchievement.archivement_id;

                if (!achievement) {
                    console.warn('Achievement data missing for user achievement:', userAchievement._id);
                    return null;
                }
                ...
                return {
                    ...
                    className: this.getAchievementClassName(achievement.condition.type),
    ...
    async getAllAchievements(): Promise<Achievement[]> {
        try {
            const data = await this.makeRequest<{ data: Achievement[] }>('/api/achievements');
            return data.data || [];
        } catch (error) {
