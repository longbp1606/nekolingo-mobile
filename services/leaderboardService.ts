...
class LeaderboardService {
    ...
    async getOverallLeaderboard(): Promise<LeaderboardUser[]> {
        try {
            const data = await this.makeRequest<LeaderboardUser[]>('/api/leaderboard/overall');
            return data;
        } catch (error) {
            throw error;
        }
    ...
    async getDetailedLeaderboard(): Promise<DetailedUser[]> {
        try {
            ...
            const detailedUsersPromises = leaderboardUsers.map(async (user, index) => {
                try {
                    ...
                    const detailedUser: DetailedUser = {
                        ...
                        score: `${user.xp} XP`,
                        xp: user.xp,
                        avatar: (userDetail.username || userDetail.email).charAt(0).toUpperCase(),
    ...
    private getRandomColor(): string {
        const colors = [
            '#ff9500', '#ff69b4', '#ff4444', '#4CAF50',
            '#9966ff', '#00BFFF', '#FFD700', '#FF6347'
        ];
    ...
    async getTournamentLeaderboards(): Promise<TournamentLeaderboards> {
        try {
            ...
            const silver = allUsers.filter(user => user.xp >= 500 && user.xp < 2000);
            const gold = allUsers.filter(user => user.xp >= 2000 && user.xp < 10000);
    ...
    async getCurrentUserRank(currentUserId: string): Promise<{ rank: number; tournament: string } | null> {
        try {
            const detailedUsers = await this.getDetailedLeaderboard();
            const userIndex = detailedUsers.findIndex(user => user._id === currentUserId);
            ...
            const tournamentUsers = tournamentData[tournament as keyof TournamentLeaderboards];
            const userInTournament = tournamentUsers.find(user => user._id === currentUserId);
