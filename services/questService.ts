...
import { FormattedQuest, QuestClaimResponse, QuestCompletionResponse, QuestDetail, QuestProgress, QuestReward, UserQuest } from '../types/quest';
...
class QuestService {
    ...
    async getDailyQuests(): Promise<UserQuest[]> {
        try {
            ...
            if (__DEV__ && error instanceof Error && error.message.includes('500')) {
    ...
    async completeQuest(questId: string): Promise<QuestCompletionResponse> {
        try {
            const data = await this.makeRequest<QuestCompletionResponse>(`/api/quest/${questId}/complete`, {
                method: 'POST',
            });
            return data;
        } catch (error) {
    ...
    async claimQuestReward(questId: string): Promise<QuestClaimResponse> {
        try {
            const data = await this.makeRequest<QuestClaimResponse>(`/api/quest/${questId}/claim`, {
                method: 'POST',
            });
            return data;
        } catch (error) {
    ...
    formatQuestForUI(userQuest: UserQuest): FormattedQuest {
        ...
        const currentProgress = quest_id.progress !== undefined ? quest_id.progress : progress;
    ...
    private getQuestSubtitle(quest: QuestDetail): string {
        ...
        switch (quest.type) {
            ...
            case 'Streak':
                ...
                break;
            case 'Result':
                subtitle = `Đạt trên ${quest.score || 95}% trong ${quest.condition} bài học`;
                break;
            default:
    ...
    async checkQuestCompletion(userQuests: UserQuest[]): Promise<QuestProgress[]> {
        ...
        const result = userQuests.map(quest => {
            const progress = quest.quest_id.progress !== undefined ? quest.quest_id.progress : (quest.progress || 0);
            return {
                questId: quest._id,
                progress: progress,
...
export type { FormattedQuest, QuestDetail, QuestProgress, QuestReward, UserQuest };
