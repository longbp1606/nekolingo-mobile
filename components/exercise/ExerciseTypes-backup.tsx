...
export const ReorderExercise: React.FC<ReorderExerciseProps> = ({
  ...
  }, [selectedSlots, availableWords, isAnswerSubmitted, isProcessing, onReorderWords, onAnswerSelect]);

  const handleSlotClick = React.useCallback((slotIndex: number) => {
    if (isAnswerSubmitted || isProcessing) return;
  ...
  }, [selectedSlots, isAnswerSubmitted, isProcessing, onReorderWords, onAnswerSelect]);
      newSlots[slotIndex] = "";
      setSelectedSlots(newSlots);
...
export const MatchExercise: React.FC<MatchExerciseProps> = ({
  ...
  const handleLeftClick = React.useCallback((leftText: string) => {
    if (isAnswerSubmitted || isProcessing) return;
  ...
  }, [matchedPairs, selectedRight, selectedLeft, isAnswerSubmitted, isProcessing, onMatchPairs]);

  const handleRightClick = React.useCallback((rightText: string) => {
    if (isAnswerSubmitted || isProcessing) return;
  ...
  }, [matchedPairs, selectedLeft, selectedRight, isAnswerSubmitted, isProcessing, onMatchPairs]);
